        ...
        <a-row :gutter="24">
          <a-col :md="24" :sm="24">
            ...
            >
              <a-textarea
                :rows="1"
                :placeholder="requestParametersPlaceholder"
                v-decorator="['requestParameters']"
              ></a-textarea>
            </a-form-item>
          </a-col>
        </a-row>
...
<script>
import { sysTimersEdit, sysTimersGetLocalJobs } from '@/api/modular/system/timersManage'
    ...
    onChangeRequestType(e) {
      ...
      if (!this.showHeaders) {
        if (this.LocalJobsDropDown.length === 0) {
          sysTimersGetLocalJobs().then(res => {
