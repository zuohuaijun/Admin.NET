    ...
    public class SysConfigSeedData : IEntitySeedData<SysConfig>
    {
        ...
        public IEnumerable<SysConfig> HasData(DbContext dbContext, Type dbContextLocator)
        {
            return new[]
            {
                ...
                new SysConfig{Id=142307070902376, Name="日志单次写入数量", Code="DILON_NUMBER_OF_SINGLE_LOG_WRITES", Value="100",SysFlag="Y", Remark="定义待写入列表的阀值，默认为100条，即列表数据大于100条时才会向数据库做一次批量写入操作", Status=0, GroupCode="DEFAULT"},
            };
        }
    }
}